...
2. **P4 chat agent — core DONE, Zalo transport DONE (deploy pending).** Loop hardened (session cartId/userId restore + injection, link_channel attaches member to open cart, cart cleared after place_order, tool responses compacted for tokens). **Web chat harness** at `http://localhost:8080/agent/ui` (simulates the Zalo channel; also the demo fallback). Requires `OPENAI_API_KEY` in `reco/.env`. **Zalo OA webhook adapter shipped** in `reco/app/agent/zalo.py` + `POST /zalo/webhook` in `main.py` (signature verify, token auto-refresh persisted to Mongo `zalo_tokens`, CS send). Creds in `reco/.env` (`ZALO_*`). **DEPLOYED & LIVE** on VPS `momolita` at `https://pawgrammers.io.vn/zalo/webhook` (nginx+TLS → reco `:8080`; backend `:3100`; systemd `kfc-reco`/`kfc-backend`). OAuth PKCE consent (`/zalo/oauth/login`) + auto-refresh working. Deploy runbook: `docs/ZALO-SETUP.md`; session walkthrough + next steps: `docs/ZALO-HANDOFF.md`. **Next up:** member identity linking (Zalo id → KFC member via `channel_identities`), then rich message replies. **Reco metrics dashboard** also added: `http://localhost:3000/api/admin/dashboard` (AOV uplift, acceptance by slot/strategy, combo trade-up; combo upgrades now logged as `combo_upgrade_accepted`).
